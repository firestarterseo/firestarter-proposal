...
import { usesLineItemInvestment } from "./proposalTypes";
...
export function mapProposalRowToDocumentData(proposal, { packages = [], addons = [], caseStudies = [] } = {}) {
  ...
  const proposalType = proposal.proposal_type || "seo";
  const lineItems = (proposal.line_items || []).map((item) => ({
    description: item.description || "",
    priceAmount: item.price_amount ?? item.priceAmount ?? 0,
    priceUnit: item.price_unit ?? item.priceUnit ?? "",
    qty: item.qty ?? 1,
    group: item.group || "primary",
  }));

  const agreementFinancials = usesLineItemInvestment(proposalType)
    ? computeLineItemFinancials(lineItems, proposal.discount_amount)
    : computeAgreementFinancials(mappedPackages, mappedAddons);

  return {
    proposalType,
    lineItems,
    investmentRecommendation: proposal.investment_recommendation || "",
    ...
    ppcKeywords: proposal.ppc_keywords || [],
    ppcAvgCpc: proposal.ppc_avg_cpc,
    ppcBudgetTiers: proposal.ppc_budget_tiers || [],
...
// Same purpose as computeAgreementFinancials above, but for General SOW /
// PPC-only / Website proposals, which price with freeform line items instead
// of catalog packages/add-ons (see lib/proposalTypes.js). There's no
// structured one-time-vs-recurring flag on a line item, so this infers it
// from the price unit: anything billed "/mo" (or "/month") is recurring,
// everything else (a flat one-time price, "/year", "/page", etc.) counts
// toward the one-time setup fee. Only line items in the "primary" group
// count — a "other_costs" group item (e.g. Website's hosting/licensing
// table) isn't part of what's being signed for in the agreement.
export function computeLineItemFinancials(lineItems, discountAmount) {
  const primary = (lineItems || []).filter((item) => (item.group || "primary") === "primary");
  const amount = (item) => {
    const price = Number(item.priceAmount) || 0;
    const qty = item.qty === "" || item.qty === undefined || item.qty === null ? 1 : Number(item.qty) || 0;
    return price * qty;
  };
  const isRecurring = (item) => /\/\s*mo(nth)?\b/i.test(item.priceUnit || "");

  const totalMonthly = primary.filter(isRecurring).reduce((sum, item) => sum + amount(item), 0);
  const setupFee = primary.filter((item) => !isRecurring(item)).reduce((sum, item) => sum + amount(item), 0);

  return {
    setupFee,
    totalMonthly: Math.max(totalMonthly - (Number(discountAmount) || 0), 0),
  };
}
