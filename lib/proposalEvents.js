...
export function describeEventGroup(group, { contactName, companyName } = {}) {
  const name = contactName?.trim() || "The client";
  const total = formatDuration(group.durationSeconds);
  // Only needed for the cross-proposal dashboard feed, where "the proposal"
  // alone is ambiguous — omitted on the per-proposal page, where it isn't.
  const proposal = companyName ? `the proposal ${companyName}` : "the proposal";

  switch (group.eventType) {
    case "viewed":
      if (group.count === 1) return total ? `${name} viewed ${proposal} for ${total}` : `${name} viewed ${proposal}`;
      return total ? `${name} viewed ${proposal} ${group.count} times, totaling ${total}` : `${name} viewed ${proposal} ${group.count} times`;
    case "sent":
      return group.count > 1 ? `You sent ${proposal} ${group.count} times` : `You sent ${proposal}`;
    case "edited":
      return group.count > 1 ? `${group.actorName || "You"} edited ${proposal} ${group.count} times` : `${group.actorName || "You"} edited ${proposal}`;
    case "accepted":
      return `${name} signed ${proposal}`;
    case "declined":
      return `${name} declined ${proposal}`;
    default:
